...
struct Compare {
	...
	void show(std::string table, std::string name) {
		for (int i = 0; i < tot; i++)
			if (table == nameLists[i]) {
				for (int j = dataLists.size() - 1; i>=0; i--)
					if (dataLists[i].tableNameLists[j] == name) {
...
public:
	...
	int calcValue() {
		int con;
		static_compare.setValue(lefIdent.table, lefIdent.ident, lefStrValue, lefIntValue, con);
		static_compare.setValue(rigIdent.table, rigIdent.ident, rigStrValue, rigIntValue, con);
		if (!valueType) {
			if (type == TREE_OP_LT) {
